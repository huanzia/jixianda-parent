<template>
  <view class="cart_page">
    ...
    <scroll-view class="cart_list" scroll-y="true" v-if="cartList.length > 0">
      <view class="cart_item" v-for="(item, index) in cartList" :key="item.id || ('cart-' + index)">
        ...
        <view class="item_main">
          ...
          <view class="item_bottom">
            ...
            <view class="item_stepper">
              <image class="step_btn" src="../../static/btn_red.png" mode="" @click="subItemByIndex(index)"></image>
              <text class="step_num">{{ item.number || 0 }}</text>
              <image class="step_btn" src="../../static/btn_add.png" mode="" @click="addItemByIndex(index)"></image>
            </view>
          </view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    addItemByIndex(index) {
      const item = Array.isArray(this.cartList) ? this.cartList[index] : null;
      this.addItem(item);
    },
    subItemByIndex(index) {
      const item = Array.isArray(this.cartList) ? this.cartList[index] : null;
      this.subItem(item);
    },
    clearCart() {
