...
export default {
	...
	computed: {
		...
		orderListDataes: function () {
			const list = this.$store && this.$store.state ? this.$store.state.orderListData : []
	...
	async onLoad(options) {
		...
		await this.refreshOrderDataFromCart()
		this.init()
		// 存在options说明换地址了
		...
		console.log('[order-init]', {
			cartSize: this.orderListDataes.length,
	...
	methods: {
		...mapState(['orderListData', 'remarkData', 'addressData', 'storeInfo', 'shopInfo', 'deliveryFee', 'warehouseId']),
		...mapMutations(['setAddressBackUrl', 'setOrderData', 'setArrivalTime', 'setRemark', 'setGender', 'initdishListMut']),
		init() {
			this.computOrderInfo()

		},
		async refreshOrderDataFromCart() {
			try {
		...
		async getEstimatedDeliveryTime() {
			...
			this.orderTime =
				...
				'-' +
				pad(nextHour.getMonth() + 1) +
				'-' +
				pad(nextHour.getDate()) +
				' ' +
				pad(nextHour.getHours()) +
				':' +
				pad(nextHour.getMinutes()) +
				':' +
				pad(nextHour.getSeconds())
			this.arrivalTime = pad(nextHour.getHours()) + ':' + pad(nextHour.getMinutes())
			console.log('[order][estimated-delivery-local]', {
				apiCalled: false,
		...
		payOrderHandle() {
			...
			const warehouseId = this.warehouseId && this.warehouseId()
			if (!warehouseId) {
				uni.showToast({
					title: '仓库信息缺失',
					icon: 'none',
			...
			console.log('[order-submit-body]', params)
			...
			submitOrderSubmit(params).then(res => {
				if (res.code === 1) {
					...
					uni.navigateTo({
						url: '/pages/pay/index?orderId=' + (res.data.orderNumber || res.data.id)
					})
				} else {
					uni.showToast({
						title: res.msg || '操作失败',
