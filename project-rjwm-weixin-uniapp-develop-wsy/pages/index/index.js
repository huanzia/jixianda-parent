...
export default {
	...
	methods: {
												...
												.then((success) => {
													if (success.code === 1) {
														...
														getNearestWarehouse({
															...
															if (nearestRes && nearestRes.code === 1 && nearestRes.data && nearestRes.data.warehouseId) {
																_this.setWarehouseId(nearestRes.data.warehouseId)
																_this.applyWarehouseInfo(nearestRes.data)
															}
														}).catch(() => { })
														// 保存配送费（后端未返回时兜底0）
														_this.setDeliveryFee(0)
														_this.init()
													}
												})
												.catch((err) => { })
			...
			await uni.getLocation({
				...
				return getMerchantInfo({
					...
					if (res && res.code === 1 && res.data) {
						this.applyWarehouseInfo(res.data)
					}
				})
			}).catch(() => { })
		},
		applyWarehouseInfo(warehouse) {
			const normalizedStatus = String(warehouse.status) === '1' ? 1 : 0
			this.phoneData = warehouse.phone || ''
			this.setShopPhone({ phone: warehouse.phone || '' })
			this.setShopStatus(normalizedStatus)
			this.shopStatus = normalizedStatus
			this.setShopInfo({
				shopName: warehouse.warehouseName || '',
				shopAddress: warehouse.address || '',
				shopId: warehouse.warehouseId || '',
				phone: warehouse.phone || '',
				status: normalizedStatus
			})
			console.log('[index][apply-warehouse]', {
				warehouseName: warehouse.warehouseName || '',
				rawStatus: warehouse.status,
				normalizedStatus,
				localShopStatus: this.shopStatus
			})
		...
		getDishNumberPreview() {
			const list = Array.isArray(this.dishListData) ? this.dishListData : []
			return list.slice(0, 5).map((item) => ({
				id: item.id,
				dishNumber: item.dishNumber || 0,
			}))
		},
		async syncCartDisplayState() {
			console.log('[cart-sync-before]', {
				cartListLength: Array.isArray(this.orderListDataes) ? this.orderListDataes.length : 0,
				totalCount: this.orderDishNumber,
				totalAmount: this.orderDishPrice,
				dishNumberPreview: this.getDishNumberPreview()
			})
			await this.getTableOrderDishListes()
			this.setOrderNum()
			this.computOrderInfo()
			console.log('[cart-sync-after]', {
				cartListLength: Array.isArray(this.orderListDataes) ? this.orderListDataes.length : 0,
				totalCount: this.orderDishNumber,
				totalAmount: this.orderDishPrice,
				dishNumberPreview: this.getDishNumberPreview()
		...
		async addDishAction(item, form) {
			const clickedItem = item && item.obj ? item.obj : item
			const sourceForm = form || (item && item.item)
			console.log('[cart-add-click]', {
				id: clickedItem && clickedItem.id,
				dishId: clickedItem && clickedItem.dishId,
				setmealId: clickedItem && clickedItem.setmealId,
				dishFlavor: clickedItem && clickedItem.dishFlavor,
				form: sourceForm
			})
			if (!clickedItem) {
				return
			...
			if (
				...
				!this.orderListDataes.some((n) => n.id == clickedItem.dishId) &&
				this.flavorDataes.length > 0
			) {
				clickedItem.flavorRemark = JSON.stringify(this.flavorDataes)
			}
			// 有sort字段是菜品
			let dishFlavorDatas = ""
			let flavorRemark = []
			if (clickedItem.flavorRemark) {
				flavorRemark = JSON.parse(clickedItem.flavorRemark)
			}
			if (clickedItem.dishFlavor !== "" && clickedItem.dishFlavor) {
				dishFlavorDatas = clickedItem.dishFlavor
			...
			if (clickedItem.type === 1) {
				params = {
					...params,
					dishId: clickedItem.id,
				}
			} else if (clickedItem.type === 2) {
				params = {
					setmealId: clickedItem.id,
				}
			} else if (sourceForm === "购物车") {
				if (clickedItem.dishId) {
					params = {
						...params,
						dishId: clickedItem.dishId,
					}
				} else {
					params = {
						setmealId: clickedItem.setmealId,
					}
				}
			}
			console.log('[cart-add-payload]', params)
			newAddShoppingCartAdd(params)
				.then(async (res) => {
					if (res.code === 1) {
						await this.syncCartDisplayState()
						this.flavorDataes = []
					}
				})
				.catch((err) => { })
		...
		async redDishAction(item, form) {
			...
			console.log('[cart-sub-payload]', params)
		...
		clearCardOrder() {
			delShoppingCart()
				.then((res) => {
					...
					if (this.dishDetailes) {
						this.dishDetailes.dishNumber = 0
					}
					if (Array.isArray(this.dishListData)) {
						this.dishListData.forEach((item) => {
							item.dishNumber = 0
						})
					}
					if (Array.isArray(this.dishListItems)) {
						this.dishListItems.forEach((item) => {
							item.dishNumber = 0
						})
					}
					this.setOrderNum()
					console.log('[index][cart-clear]', {
						cartListLength: this.orderListDataes.length,
						totalAmount: this.orderDishPrice,
						totalCount: this.orderDishNumber,
						clearedFields: ['orderListData', 'orderDishNumber', 'orderDishPrice', 'tablewareNumber', 'dishNumber']
					})
					// 调用一次购物车集合---初始化
					this.getTableOrderDishListes()
					// 重新调取刷新右侧具体菜品列表
					if (this.rightIdAndType && this.rightIdAndType.id) {
