...
极鲜达（Fresh-Link）是一个面向“多仓即时零售”场景的多模块项目，包含用户端小程序、管理端前端和 Java 后端服务。项目重点展示从商品浏览、购物车、下单、库存处理到支付与超时取消的一条完整业务链路，适合作为学习、演示和多端协同开发参考。
...
- 开源仓库只保留示例配置；请先阅读 `OPEN_SOURCE_SETUP.md`，再创建你自己的私有配置文件。
...
因为本仓库面向开源展示，实际运行前需要补齐本地环境配置与中间件依赖。

### 为什么需要多个服务？

该项目通过网关、用户服务、核心业务服务拆分职责，便于展示多模块协作方式。

### 为什么默认保留 mock 支付？

为了降低本地演示门槛，避免依赖真实支付商户环境。
...
- Issue / PR 规范见 `CONTRIBUTING.md`
- 安全问题提交流程见 `SECURITY.md`
- 社区行为准则见 `CODE_OF_CONDUCT.md`

## 15. 免责声明

- 本项目仅供学习交流与技术展示使用。
- 如仓库中涉及第三方素材、图标或资源，请在商用前自行核验授权。
- 请勿将示例配置直接用于生产环境。
